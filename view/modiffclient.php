...
<?php if (isset($_POST['nom'])) : 
        $requete = new modifpatient (htmlspecialchars($_POST['nom']), htmlspecialchars($_POST['prenom']), htmlspecialchars($_POST['email']), htmlspecialchars($_POST['age']), htmlspecialchars($_POST['poid']), htmlspecialchars($_POST['taille']) );?>
        <div class="alert alert-success" role="alert" style="margin: 0%;">
            <strong>Vous avez bien modiffier <?= $_POST['nom']?></strong> 
        </div>
    <?php endif ?>
            ...
            <tbody>
                ...
                foreach ($result as $range) : ?>
                    <tr style="padding: 20px;">
                        ...
                        <td><a class="btn btn-warning" href="modif.php?key=<?=$range["id"]?>"role="button">change</a></td>
                        <td><a class="btn btn-danger" href="delete.php?key=<?=$range["id"]?>" role="button">delete</a></td>
